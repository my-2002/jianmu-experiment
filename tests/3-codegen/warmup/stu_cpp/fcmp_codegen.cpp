...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("lu12i.w $t3, 0x40b00");
    codegen->append_inst("movgr2fr.w $ft0, $t3");
    // TODO: 将 1.0 (0x3f800000) 加载到浮点寄存器中
    codegen->append_inst("lu12i.w $t3, 0x3f800");
    codegen->append_inst("movgr2fr.w $ft1, $t3");
    ...
    codegen->append_inst("st.d",{"$t0", "$fp", std::to_string(offset_map["%op0"])});
    ...
    codegen->append_inst("slt $t1, $zero, $t0");
    codegen->append_inst("st.w",{"$t1", "$fp", std::to_string(offset_map["%op2"])});
    ...
    codegen->append_inst("addi.w $t1, $zero,0");
    codegen->append_inst("ld.w",{"$t0", "$fp", std::to_string(offset_map["%op2"])});
    codegen->append_inst("bne $t0, $t1, .main_label3");
