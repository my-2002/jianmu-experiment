...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("addi.w $t1, $t1, 0x5c3");
    codegen->append_inst("movgr2fr.w $ft0, $t1");
    // TODO: 将通用寄存器或者浮点寄存器中的值写入 %op0 对应的内存空间中
    codegen->append_inst("fst.s $ft0, $t0, 0");
    ...
    codegen->append_inst("ld.d", {"$t0", "$fp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("fld.s $ft0, $t0, 0");
    codegen->append_inst("fst.s",{"$ft0", "$fp", std::to_string(offset_map["%op1"])});
    ...
    codegen->append_inst("fld.s",{"$ft0", "$fp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("ftintrz.w.s $ft1, $ft0");
    codegen->append_inst("movfr2gr.s $t0, $ft1");
    ...
    codegen->append_inst("ld.w",{"$a0", "$fp", std::to_string(offset_map["%op2"])});
